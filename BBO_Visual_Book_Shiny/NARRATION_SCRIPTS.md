...
This page introduces the Black Box Resolution method, or B B R.

Black box optimisation begins with a difficult limitation. We can observe the inputs and returned outputs, but we cannot see the original equation, its gradients or its true optimum. B B R provides a structured way to learn as much as possible from the evidence without claiming to know more than the data can support.

The diagram across the top presents B B R as a repeating cycle.

The first stage is Evaluate. Here, the returned inputs and outputs are reviewed carefully. The purpose is to identify improvement, deterioration, repetition, instability and any result that challenges the current interpretation.

The second stage is Resolve. This separates what is supported by evidence from what remains uncertain. A promising pattern may justify another test, but it should not be treated as a recovered equation.

Explore then considers a wider or less familiar region. Exploration can reveal new information, although it may move away from a strong local result.

Exploit takes the opposite approach. It uses smaller movements to refine a region that already appears productive. This may improve performance, but it can also miss a stronger region elsewhere.

Extend asks what further question can be tested from the evidence. This may involve another coordinate, a different model, a repeated observation or a comparison between competing explanations.

Optimise selects the next coordinate using the strongest information currently available. The selected point remains a proposal until the black box returns its actual output.

Evolve means adapting the method when new evidence changes the interpretation. A useful analytical process should not continue following a rule after the observed results have contradicted it.

The final stage is Experiment. The next reproducible test is carried out, the returned result is recorded, and the process returns to Evaluate. B B R is therefore a continuing evidence cycle rather than a single fitted model.

The panels below summarise three central decisions.

Exploration buys information about untested regions but risks leaving a strong local area.

Exploitation refines a supported region using smaller movements, although it may overlook a separate and better optimum.

Confirmation repeats a coordinate or closely related test to examine stability. This strengthens confidence in a result, but every repeat consumes part of the limited evaluation budget.

The Black Box Resolution panel states the interpretation boundary. B B R compares explanations, tests their chronological performance and rejects explanations that fail. It may identify a best-supported local structure, but it does not claim recovery of the unknown original equation or its global optimum.
...
## 28. Resolution home page

Resolution is the home of Black Box Resolution, abbreviated as B B R. This route begins after the assessed optimisation challenge and asks how much hidden mathematical structure can be recovered from the completed coordinate and output record.

A black box reveals what was submitted and what was returned, but not the equation that connects them. Many different mathematical explanations can fit a limited set of observations. B B R therefore treats resolution as a process of comparison and falsification rather than a single act of curve fitting.

The process starts by evaluating the evidence already available. A candidate explanation must account for strong and weak observations, coordinate movement and the timing of the results. It must also respect the fact that the functions have different dimensions, numerical scales and observed behaviours.

The next step is to resolve what is known and what remains uncertain. A model may describe one supported region well without representing the entire hidden landscape. B B R makes that distinction explicit. A useful local surrogate is not automatically the original equation and does not establish the unknown global optimum.

Exploration and exploitation then become analytical choices. Broader movement can reveal whether a proposed explanation extends beyond a local area. Smaller movement can refine a region already supported by evidence. Repeated coordinates can test stability, although every repeat consumes part of the finite evaluation budget.

The method continues by extending the question, selecting the next coordinate, adapting to contradictory evidence and running a reproducible test. Candidate explanations that fail chronological comparison are rejected. Supported explanations are retained provisionally and remain open to further testing.

This cycle is summarised on the B B R method page as Evaluate, Resolve, Explore, Exploit, Extend, Optimise, Evolve and Experiment. It is a practical reasoning framework for hidden functions rather than a claim that one modelling family must always be used.

The completed project provides two representative outcomes. Function Five is described by a Matérn two point five Gaussian process surrogate, which can represent a smooth but flexible local surface. Function Seven is described by a quadratic surrogate, which captures the supported curvature in the available evidence.

These equations are evidence-based approximations. Their value lies in showing that the completed black box record contains recoverable mathematical behaviour. Their limitations are equally important. They do not prove that the hidden functions were generated by those equations outside the observed region.

Use Resolution home as the entrance to this research route. Select BBR method to see the full eight-stage cycle, its interpretation and the distinction between exploration, exploitation and confirmation. Follow the repository links when the complete surrogate equations, coefficients, scaling and validation results are required.
