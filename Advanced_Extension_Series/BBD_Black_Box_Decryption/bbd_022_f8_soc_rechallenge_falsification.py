...
def select_diverse(df: pd.DataFrame, xcols: list[str], n: int = 10, min_sep: float = 0.35) -> pd.DataFrame:
    chosen = []
...
def main():
    ...
    roster = [("BBD021_linear_OLS", bbd_linear())]
    ...
    comp.to_csv(OUT / "BBD_022_F8_DIRECT_SOC_RECHALLENGE.csv", index=False)
    pd.concat(pred_tables, ignore_index=True).to_csv(OUT / "BBD_022_F8_RECHALLENGE_PREDICTIONS.csv", index=False)

    bbd_row = comp[comp["model"] == "BBD021_linear_OLS"].iloc[0]
    soc_comp = comp[comp["model"].str.startswith("SOC_")].copy()
    best_soc = soc_comp.iloc[0]
    direct_winner = "BBD021_linear_OLS" if bbd_row["normalised_walk_forward_mae"] <= best_soc["normalised_walk_forward_mae"] else str(best_soc["model"])

    # Fit the BBD linear candidate and the three strongest SOC families on all historical F8 observations.
    finalist_names = ["BBD021_linear_OLS"] + soc_comp.head(3)["model"].tolist()
    estimator_map = dict(roster)
    fitted = {}
    for name in finalist_names:
        m = clone(estimator_map[name])
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")
            m.fit(X, y)
        fitted[name] = m

    # Prospective search only. No synthetic points are treated as evaluator observations.
    sobol = qmc.Sobol(d=D, scramble=True, seed=RANDOM_STATE)
    candidates = sobol.random_base2(m=13)  # 8192 points
    corners = np.array(np.meshgrid(*[[0.0, 1.0]] * D)).T.reshape(-1, D)
    C = np.vstack([candidates, corners])
    ...
    selected = select_diverse(qdf, xcols, n=10, min_sep=0.35)
    ...
    selected.to_csv(OUT / "BBD_022_F8_DISCRIMINATORY_QUERIES.csv", index=False)

    top = selected.iloc[0]
    summary = pd.DataFrame([{
        "function": F,
        "bbd021_linear_normalised_walk_forward_mae": float(bbd_row["normalised_walk_forward_mae"]),
        ...
        "interpretation": "direct_same_protocol_rechallenge_plus_prospective_falsification",
    }])
    summary.to_csv(OUT / "BBD_022_F8_RECHALLENGE_FALSIFICATION_SUMMARY.csv", index=False)

    print("BBD 022 F8 direct SOC rechallenge and discriminatory falsification")
