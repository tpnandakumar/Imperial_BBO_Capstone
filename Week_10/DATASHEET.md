...
A central motivation is therefore traceability. The repository records not only which query was submitted but also what result was returned, how that result compared with earlier observations and how it influenced the next decision. Week 10 is particularly useful because it contains both successful local movements and clear counterexamples to continuation in previously tested directions.

The dataset also supports reproducible study of exploration, refinement, exploitation and reassessment under a restricted query budget. It provides a practical record of how optimisation strategy changes when evidence strengthens, weakens or becomes contradictory.
...
The dataset is sequential rather than independently sampled. Later observations were selected using evidence available from earlier rounds, so the sampling distribution changes as the search becomes more informed. This adaptive structure is an inherent feature of the capstone rather than a source of error.

By the completion of Week 10, ten observations were available for every function. The cumulative dataset therefore contained eighty query vectors and eighty returned objective values.
...
The exact returned outputs were:
...
The exact changes were calculated using decimal arithmetic in the Week 10 analytical workflow.
...
This differentiated approach is a key characteristic of the dataset. Strategy labels describe the decision made for a particular round and are not fixed properties of a function.

## 10. Optimisation Workflow

The workflow used the following sequence:
...
Candidate query selection
        |
        v
Dimensionality and range validation
        |
        v
Official submission
        |
...
Returned objective values
        |
        v
Exact comparison and ranking
        |
        v
Strategy revision for the next round
```

The workflow preserves a clear distinction between observed data and interpretation.
...
Preprocessing was limited to structural validation, parsing for analysis and generation of derived summaries. Numerical conversion for plotting does not replace the stored source values.
...
The Week 10 scripts use exact decimal comparison for changes between Weeks 09 and 10. This avoids unnecessary rounding in the stored analytical summaries. Raw CSV files remain separate from figures and narrative interpretation.
...
This classification is consistent with the Week 10 README and analytical files.

## 16. Exploratory Data Characteristics

The dataset contains a mixture of broad exploratory observations and increasingly concentrated local observations. This reflects the normal progression of sequential optimisation. Productive regions receive more attention over time, while unresolved functions retain broader search behaviour.
...
The primary data originate from two sources: query vectors submitted through the official BBO platform and objective values returned by that platform. The repository stores those values in `week_10_inputs.csv` and `week_10_results.csv`.

Analytical summaries and figures are derived from those source files and from the verified Week 09 comparison data.

## 20. Data Integrity

Source observations are not overwritten during analysis. Derived outputs are written to separate summary files. This separation allows the repository to preserve the exact competition record while supporting repeated analysis.
...
The analytical scripts and summaries should agree with these files but do not supersede them.
...
This is appropriate for optimisation but limits the use of the dataset for claims about the global geometry of the hidden functions. Absence of observations in a region does not imply poor performance there.

## 22. Limitations

The main limitation is that the true functions and global optima are unknown. Ten observations per function remain sparse, particularly for the higher dimensional objectives. Apparent trends may therefore represent local behaviour rather than global structure.

Cross function numerical rankings should also be interpreted cautiously because the eight objectives operate on different scales. Ranking is descriptive and does not imply that the functions are directly comparable as tasks.
...
It also supports audit of how each submitted query was influenced by evidence available at the time.
...
The capstone dataset contains optimisation values rather than personal or clinical records. The principal responsible use issues therefore concern transparency, reproducibility and appropriate limits on claims.
