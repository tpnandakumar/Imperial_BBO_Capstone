"""
Week 10 Analysis Tool
Imperial BBO Capstone

This script validates the Week 10 input and result files, compares the
Week 10 outputs with Week 09, ranks all eight objective functions and
exports week_10_analysis_summary.csv.
...
WEEK_10_INPUTS_FILE = CURRENT_DIRECTORY / "week_10_inputs.csv"
WEEK_10_RESULTS_FILE = CURRENT_DIRECTORY / "week_10_results.csv"
WEEK_10_SUMMARY_FILE = CURRENT_DIRECTORY / "week_10_analysis_summary.csv"

WEEK_09_RESULTS_FILE = (
    CURRENT_DIRECTORY.parent / "Week_09" / "week_09_results.csv"
)
...
def validate_inputs_file() -> None:
    """Validate the Week 10 input vectors and their dimensions."""

    if not WEEK_10_INPUTS_FILE.exists():
        raise FileNotFoundError(f"Input file not found: {WEEK_10_INPUTS_FILE}")

    rows: Dict[str, List[str]] = {}

    with WEEK_10_INPUTS_FILE.open(
        "r",
        encoding="utf-8-sig",
        newline="",
    ) as csv_file:
        ...
        if reader.fieldnames is None:
            raise ValueError(
                f"No CSV header found in {WEEK_10_INPUTS_FILE.name}"
            )

        required_columns = {"Function", "Input"}
        if not required_columns.issubset(set(reader.fieldnames)):
            raise ValueError(
                f"{WEEK_10_INPUTS_FILE.name} must contain the columns "
                "'Function' and 'Input'."
            )
    ...
    if set(rows) != set(EXPECTED_DIMENSIONS):
        raise ValueError(
            f"{WEEK_10_INPUTS_FILE.name} must contain Functions 1 to 8."
        )
...
def classify_direction(change: Decimal) -> str:
    """Classify the exact change relative to Week 09."""
...
def build_summary(
    ...
    """Rank Week 10 functions and build the exact comparison summary."""

    ranked_functions = sorted(
        week_10_results.items(),
    ...
    for rank, (function_name, values) in enumerate(
        ...
        week_09_value = week_09_results[function_name]["output_decimal"]
        week_10_value = values["output_decimal"]
        change = week_10_value - week_09_value
        ...
        summary_rows.append(
            {
                ...
                "Status": classify_status(week_10_value),
...
def write_summary(summary_rows: List[Dict[str, object]]) -> None:
    """Export the Week 10 analysis summary without changing source values."""
    ...
    with WEEK_10_SUMMARY_FILE.open(
        "w",
        encoding="utf-8",
        newline="",
    ) as csv_file:
...
def print_summary_report(summary_rows: List[Dict[str, object]]) -> None:
    """Print the Week 10 ranking and strategy report."""

    best = summary_rows[0]
    worst = summary_rows[-1]

    print("\nWeek 10 BBO Analysis Report")
    print("=" * 76)
    ...
    print("\nWeek 10 ranking and comparison")
    print("-" * 76)
    ...
    print(
        ...
        f"{WEEK_10_SUMMARY_FILE.name}"
    )


def main() -> None:
    """Run the complete Week 10 analysis."""

    validate_inputs_file()
    week_09_results = read_results(WEEK_09_RESULTS_FILE)
    week_10_results = read_results(WEEK_10_RESULTS_FILE)
    summary_rows = build_summary(week_09_results, week_10_results)
